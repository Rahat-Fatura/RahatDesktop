...
const createInvoiceJson = async (id) => {
  ...
  for (let i = 0; i < companyConfig.settings.numberOfRetries; i += 1) {
    const queryResults = await query.runAllInvoiceQuery(id);
    ...
    if (await queue.isValid()) {
      ...
      await sleep(companyConfig.settings.queueCreatorWaitingMs);
    }
  }
  if (!invoiceJson) {
